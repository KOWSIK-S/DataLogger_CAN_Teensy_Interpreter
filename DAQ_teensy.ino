 * @file motostudent_logger_v5.0.ino
 * @brief Final Production-Grade, High-Reliability CAN Logger for MotoStudent
 * @version 5.0 (Final-Robust)
 * @details
 * This firmware is the definitive version for the Teensy 4.1-based MotoStudent data logger.
 * It incorporates a comprehensive set of features for maximum performance, data integrity,
 * and long-term reliability in a harsh motorsports environment.
 * @version_history
 * v5.0 :
 * - GUARANTEED FILENAME UNIQUENESS: Appends an incrementing index to filenames (e.g., ..._000.csv)
 * to prevent overwrites, even with a non-battery-backed RTC.
 * - STRICT CSV CONSISTENCY: Data rows now contain a placeholder for the message column,
 * ensuring every row has an identical number of fields for easy parsing.
 * - CAN BUS HEALTH MONITORING: Actively monitors the CAN controller state and logs
 * any errors (Warning, Bus-Off) as critical system events.
 * v4.0 :
 * - Expanded dataset to include Oil P/T, IAT, TC Slip, and Ignition Angle.
 * v3.0:
 * - Implemented RTC-based filenames, decoupled SD flushing, and advanced event/LED logic.
 */

#include <FlexCAN_T4.h>
#include <SdFat.h>
#include <WDT_T4.h>
#include <CRC32.h>
#include <TimeLib.h> // Required for RTC functionality

// -------------------- System Configuration --------------------
constexpr int LED_PIN = 13;
constexpr int CHIP_SELECT = BUILTIN_SDCARD;
constexpr int SD_CLK_MHZ = 50;
constexpr int CAN_BAUD_RATE = 500000; // 500 kbit/s for AIM CAN bus

#define CSV_HEADER "Type,Timestamp(ms),RPM,TPS(%%),Coolant(C),MAP(mBar),VBAT(V),Gear,Lambda1,OilPress(bar),OilTemp(C),IAT(C),Slip(%%),IgnAngle(deg),CRC32,Message\n"

// -------------------- Reliability & Performance Settings --------------------
constexpr uint32_t WDT_TIMEOUT_MS = 4000;         // Watchdog timeout (4 seconds)
constexpr uint32_t CAN_TIMEOUT_MS = 500;          // Time to wait for a CAN message before flagging a timeout
constexpr uint32_t LOG_INTERVAL_MS = 20;          // Log data to buffer every 20ms (50 Hz)
constexpr uint32_t FLUSH_INTERVAL_MS = 500;       // "Hard flush" SD card cache every 500ms
constexpr uint32_t FILE_SIZE_LIMIT = 25L * 1024L * 1024L; // 25MB limit per log file
constexpr size_t LOG_BUFFER_SIZE = 8192;          // 8KB RAM buffer for log lines

// -------------------- CAN Bus Message IDs (from AIM Protocol) --------------------
constexpr uint32_t ID_RPM_TPS = 0x5F0;
constexpr uint32_t ID_TEMPS = 0x5F2;
constexpr uint32_t ID_PRESSURES = 0x5F3;
constexpr uint32_t ID_VBAT_GEAR = 0x5F4;
constexpr uint32_t ID_LAMBDA = 0x5F6;
constexpr uint32_t ID_ADVANCED = 0x5FB; // Contains Slip and Ignition Angle

// -------------------- Global Objects & Variables --------------------
FlexCAN_T4<CAN1, RX_SIZE_256, TX_SIZE_16> Can0;
SdFat sd;
SdFile logfile;
WDT_T4<WDT1> wdt;

struct CAN_Data {
  int rpm = -1; float tps = -1.0f; float ect = -1.0f;
  float map_mbar = -1.0f; float vbat = -1.0f; int gear = -1;
  float lambda1 = -1.0f; float oil_press_bar = -1.0f; float oil_temp_c = -1.0f;
  float iat_c = -1.0f; float slip_percent = -1.0f; float ign_angle_deg = -99.0f;
};
CAN_Data currentData;

char logBuffer[LOG_BUFFER_SIZE];
size_t bufferIndex = 0;
char filename[32];
static uint16_t fileCreationIndex = 0; // Guarantees unique filenames across power cycles/resets

uint32_t lastLogTime = 0;
uint32_t lastFlushTime = 0;
uint32_t lastCANMessageTime = 0;
bool canTimeout = true;


// -------------------- Function Declarations --------------------
void setupRTC();
void newLogFile();
void flushBuffer();
void updateCANData(const CAN_message_t &msg);
void logDataPacket();
void logSystemEvent(const char* type, const char* message);
void checkCANBusHealth();
void updateLedStatus();
void safeDelay(uint32_t ms);
void systemHalt(const char* reason);


// -------------------- Setup Function --------------------
void setup() {
  pinMode(LED_PIN, OUTPUT);
  digitalWrite(LED_PIN, LOW);

  Serial.begin(115200);
  safeDelay(2000);
  Serial.println("\n--- MotoStudent Rugged CAN Logger v5.0 (Final-Robust) ---");
  Serial.println("Initializing...");

  setupRTC();
  
  Can0.begin();
  Can0.setBaudRate(CAN_BAUD_RATE);
  Serial.println("CAN bus initialized.");

  if (!sd.begin(CHIP_SELECT, SD_SCK_MHZ(SD_CLK_MHZ))) {
    systemHalt("SD Card init failed!");
  }
  Serial.println("SD card initialized.");
  
  newLogFile();
  logSystemEvent("INFO", "System Initialized; Logging Started");

  WDT_timings_t config;
  config.timeout = WDT_TIMEOUT_MS;
  wdt.begin(config);
  Serial.println("Watchdog timer enabled.");

  uint32_t now = millis();
  lastCANMessageTime = now;
  lastLogTime = now;
  lastFlushTime = now;

  Serial.println("System initialization complete.");
}


// -------------------- Main Loop --------------------
void loop() {
  wdt.feed();

  CAN_message_t msg;
  while (Can0.read(msg)) {
    if (canTimeout) {
      canTimeout = false;
      logSystemEvent("INFO", "CAN Bus Communication Resumed");
    }
    lastCANMessageTime = millis();
    updateCANData(msg);
  }

  if (!canTimeout && (millis() - lastCANMessageTime > CAN_TIMEOUT_MS)) {
    canTimeout = true;
    logSystemEvent("ERROR", "CAN Bus Timeout");
    currentData = CAN_Data();
  }

  if (millis() - lastLogTime >= LOG_INTERVAL_MS) {
    lastLogTime = millis();
    logDataPacket();
  }

  if (millis() - lastFlushTime >= FLUSH_INTERVAL_MS) {
    lastFlushTime = millis();
    flushBuffer();
  }

  checkCANBusHealth();
  updateLedStatus();
}


// -------------------- Core Functions --------------------

void updateCANData(const CAN_message_t &msg) {
  switch (msg.id) {
    case ID_RPM_TPS:
      currentData.rpm = (msg.buf[1] << 8) | msg.buf[0];
      currentData.tps = ((msg.buf[3] << 8) | msg.buf[2]) / 650.0f;
      break;
    case ID_TEMPS:
      currentData.iat_c = ((((msg.buf[1] << 8) | msg.buf[0]) / 19.0f) - 450.0f) / 10.0f;
      currentData.ect = ((((msg.buf[3] << 8) | msg.buf[2]) / 19.0f) - 450.0f) / 10.0f;
      currentData.oil_temp_c = ((((msg.buf[7] << 8) | msg.buf[6]) / 19.0f) - 450.0f) / 10.0f;
      break;
    case ID_PRESSURES:
      currentData.map_mbar = ((msg.buf[1] << 8) | msg.buf[0]) / 10.0f;
      currentData.oil_press_bar = ((msg.buf[5] << 8) | msg.buf[4]) / 100.0f;
      break;
    case ID_VBAT_GEAR:
      currentData.vbat = (((msg.buf[3] << 8) | msg.buf[2]) / 32.0f) / 100.0f;
      currentData.gear = (int16_t)((msg.buf[7] << 8) | msg.buf[6]);
      break;
    case ID_LAMBDA:
      currentData.lambda1 = (((msg.buf[1] << 8) | msg.buf[0]) / 2.0f) / 1000.0f;
      break;
    case ID_ADVANCED:
      currentData.slip_percent = ((msg.buf[3] << 8) | msg.buf[2]) / 650.0f;
      currentData.ign_angle_deg = (int16_t)((msg.buf[5] << 8) | msg.buf[4]) / 3.0f / 100.0f;
      break;
  }
}

void logDataPacket() {
  char tempLine[256];
  
  int len = snprintf(tempLine, sizeof(tempLine), "DATA,%lu,%d,%.2f,%.1f,%.1f,%.2f,%d,%.3f,%.2f,%.1f,%.1f,%.2f,%.2f",
                     millis(), currentData.rpm, currentData.tps, currentData.ect,
                     currentData.map_mbar, currentData.vbat, currentData.gear, currentData.lambda1,
                     currentData.oil_press_bar, currentData.oil_temp_c, currentData.iat_c,
                     currentData.slip_percent, currentData.ign_angle_deg);

  uint32_t crc = CRC32::calculate(tempLine, len);
  
  // Append CRC and a placeholder for the message column to ensure consistent column count
  len += snprintf(tempLine + len, sizeof(tempLine) - len, ",%lu,,\n", crc);

  if ((bufferIndex + len) >= LOG_BUFFER_SIZE) { flushBuffer(); }
  memcpy(logBuffer + bufferIndex, tempLine, len);
  bufferIndex += len;

  if (logfile.size() > FILE_SIZE_LIMIT) {
    flushBuffer();
    logfile.close();
    newLogFile();
  }
}

void logSystemEvent(const char* type, const char* message) {
  char tempLine[256];
  int len = snprintf(tempLine, sizeof(tempLine), "%s,%lu,,,,,,,,,,,,,,%s\n", type, millis(), message);

  if ((bufferIndex + len) >= LOG_BUFFER_SIZE) { flushBuffer(); }
  memcpy(logBuffer + bufferIndex, tempLine, len);
  bufferIndex += len;
}

void checkCANBusHealth() {
    static FLEXCAN_BUS_STATE lastBusState = F_BUS_NORMAL;
    FLEXCAN_BUS_STATE currentBusState = Can0.busState();

    if (currentBusState != lastBusState) {
        char msg[64];
        switch (currentBusState) {
            case F_BUS_WARNING:
                snprintf(msg, sizeof(msg), "CAN Bus State Changed to WARNING (err count: %d)", Can0.errorCount());
                logSystemEvent("WARN", msg);
                break;
            case F_BUS_ERROR:
                 snprintf(msg, sizeof(msg), "CAN Bus State Changed to ERROR (err count: %d)", Can0.errorCount());
                logSystemEvent("ERROR", msg);
                break;
            case F_BUS_BUS_OFF:
                snprintf(msg, sizeof(msg), "CAN Bus State Changed to BUS-OFF");
                logSystemEvent("CRITICAL", msg);
                break;
            case F_BUS_NORMAL:
                snprintf(msg, sizeof(msg), "CAN Bus State Returned to NORMAL");
                logSystemEvent("INFO", msg);
                break;
        }
        lastBusState = currentBusState;
    }
}

// -------------------- Utility Functions --------------------

void setupRTC() {
  auto parseMonth = [](const char* month) -> int {
    if (strcmp(month, "Jan") == 0) return 1; if (strcmp(month, "Feb") == 0) return 2;
    if (strcmp(month, "Mar") == 0) return 3; if (strcmp(month, "Apr") == 0) return 4;
    if (strcmp(month, "May") == 0) return 5; if (strcmp(month, "Jun") == 0) return 6;
    if (strcmp(month, "Jul") == 0) return 7; if (strcmp(month, "Aug") == 0) return 8;
    if (strcmp(month, "Sep") == 0) return 9; if (strcmp(month, "Oct") == 0) return 10;
    if (strcmp(month, "Nov") == 0) return 11; if (strcmp(month, "Dec") == 0) return 12;
    return 0;
  };

  char dateStr[] = __DATE__; char timeStr[] = __TIME__;
  char monthStr[4]; int day, year;
  sscanf(dateStr, "%s %d %d", monthStr, &day, &year);

  tmElements_t tm;
  tm.Year = CalendarYrToTm(year); tm.Month = parseMonth(monthStr); tm.Day = day;
  sscanf(timeStr, "%d:%d:%d", &tm.Hour, &tm.Minute, &tm.Second);

  Teensy3Clock.set(makeTime(tm));
  setSyncProvider(Teensy3Clock.get);
  
  Serial.printf("RTC initialized to: %d-%02d-%02d %02d:%02d:%02d\n", year, tm.Month, day, tm.Hour, tm.Minute, tm.Second);
}

void newLogFile() {
  snprintf(filename, sizeof(filename), "%d%02d%02d_%02d%02d%02d_%03d.csv",
           year(), month(), day(), hour(), minute(), second(), fileCreationIndex++);

  if (!logfile.open(filename, O_WRONLY | O_CREAT | O_APPEND)) {
    systemHalt("Log file creation failed!");
  }

  Serial.print("Logging to new file: ");
  Serial.println(filename);

  bufferIndex = snprintf(logBuffer, LOG_BUFFER_SIZE, CSV_HEADER);
  flushBuffer();
}

void flushBuffer() {
  if (bufferIndex > 0) {
    digitalWrite(LED_PIN, HIGH);
    logfile.write(logBuffer, bufferIndex);
    bufferIndex = 0;
    logfile.sync();
    digitalWrite(LED_PIN, LOW);
  }
}

void updateLedStatus() {
  static uint32_t lastBlinkTime = 0;
  static bool ledState = false;

  if (canTimeout) {
    digitalWrite(LED_PIN, HIGH); // Solid ON for CAN Timeout
    return;
  }

  if (millis() - lastBlinkTime > (ledState ? 150 : 2000)) {
    ledState = !ledState;
    digitalWrite(LED_PIN, ledState);
    lastBlinkTime = millis();
  }
}

void safeDelay(uint32_t ms) {
    uint32_t start = millis();
    while (millis() - start < ms) {
        wdt.feed();
        yield();
    }
}

void systemHalt(const char* reason) {
  Serial.print("FATAL ERROR: ");
  Serial.println(reason);
  while (1) {
    digitalWrite(LED_PIN, HIGH);
    safeDelay(150);
    digitalWrite(LED_PIN, LOW);
    safeDelay(150);
  }
}
